...
export default function GISMap({ 
  ...
  const isNationalView = role === 'MINISTRY';
  const isStateView = role === 'STATE';
  const isProjectRadiusView = !!targetProject && (role === 'MP' || role === 'DISTRICT');
  ...
  const getInitialCenter = () => {
    if (isNationalView) return { lat: 21.7679, lon: 78.8718, zoom: 5 }; // Center of India
  ...
  useEffect(() => {
    ...
    // =========================================================================
    if (isProjectRadiusView || targetProject) {
  ...
  }, [lat, lon, nearbyProjects, activeRadius, allProjects, filterTier, role]);
          ...
          {isNationalView || isStateView ? (
            <>
              <div className="flex items-center gap-2 text-slate-700 font-medium">
                ...
                <span>Critical Risk Works (Score >= 80)</span>
              </div>
              <div className="flex items-center gap-2 text-slate-700 font-medium">
...
with open(os.path.join(BASE, "GISMap.jsx"), "w", encoding="utf-8") as f:
    f.write(code.strip())

print("GISMap.jsx updated with Pan-India National, State, and Radius support.")