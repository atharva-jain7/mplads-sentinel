...
# ReportPage.jsx
save("pages/ReportPage.jsx", """import React, { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { Printer, ArrowLeft, ShieldCheck, CheckCircle2, AlertTriangle, Building, FileText } from 'lucide-react';
...
export default function ReportPage() {
  ...
  if (loading || !report) {
    return <div className="p-12 text-center text-slate-500">Generating report...</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="bg-white border border-slate-300 p-8 sm:p-12 shadow-lg rounded-xl print:border-none print:shadow-none print:p-0 space-y-6 text-slate-900 font-sans">
        <div className="border-b-2 border-slate-900 pb-4 text-center space-y-1">
          <div className="text-xs font-bold uppercase tracking-widest text-slate-600">GOVERNMENT OF INDIA</div>
          <div className="text-xs font-semibold text-slate-700">MINISTRY OF STATISTICS AND PROGRAMME IMPLEMENTATION</div>
          <div className="text-xs font-semibold text-slate-700">MEMBERS OF PARLIAMENT LOCAL AREA DEVELOPMENT SCHEME (MPLADS)</div>
          <h2 className="text-lg font-black tracking-tight text-slate-900 pt-2 uppercase">
            CONFIDENTIAL PROJECT INVESTIGATION DOSSIER
          </h2>
          <div className="flex justify-between text-[11px] text-slate-500 font-mono pt-2 border-t border-slate-200 mt-2">
        ...
        <div className="space-y-2">
          <h3 className="text-xs font-bold uppercase tracking-wider bg-slate-100 p-1.5 text-slate-800 border-l-4 border-slate-800">
        ...
        <div className="space-y-2">
          <h3 className="text-xs font-bold uppercase tracking-wider bg-slate-100 p-1.5 text-slate-800 border-l-4 border-slate-800">
            4. RISK FUSION ASSESSMENT & PRIORITY RATING
          </h3>
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between">
            <div>
              <span className="text-[10px] text-red-700 font-bold uppercase block">COMPOSITE RISK INDEX</span>
              <span className="text-2xl font-black font-mono text-red-700">{d.riskScore || 94} / 100</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] text-red-700 font-bold uppercase block">INVESTIGATION PRIORITY</span>
              <span className="text-base font-bold text-red-800 uppercase">{d.investigationPriority || 'CRITICAL'}</span>
            </div>
          </div>
        </div>
          ...
          <ul className="space-y-1.5 text-xs">
            {(d.factors || []).map((f, i) => (
              <li key={i} className="flex items-start gap-2">
                ...
                <span><b>[{f.type} - {f.severity}]:</b> {f.explanation}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="space-y-2">
          ...
          <ol className="list-decimal pl-4 text-xs space-y-1 text-slate-800">
        ...
        <div className="pt-6 border-t border-slate-300 space-y-6">
          <div className="flex justify-between items-end text-xs">
            <div>
              <p className="text-[11px] text-slate-500">Report Transmitted Electronically via MPLADS Sentinel</p>
              <p className="font-semibold text-slate-800">MoSPI Monitoring Division</p>
            </div>
            <div className="text-right">
              <div className="h-10 w-32 border-b border-slate-400 mb-1" />
              <p className="font-bold text-slate-900">{report.generatedBy || 'Rajesh Sharma'}</p>
              <p className="text-[10px] text-slate-500">{report.designation || 'Deputy Commissioner / Nodal Officer'}</p>
            </div>
          </div>

          <div className="p-3 bg-slate-100 rounded text-[10px] text-slate-600 text-center font-medium border border-slate-200">
